...
public class PerformanceServices {
  ...
  public Performance getPerformance(Integer id) throws EntityNotFoundException, InvalidParameterException {
    if (id == null) {
  ...
  public void deletePerformance(int id) throws EntityNotFoundException, InvalidParameterException {
    ...
    if (performance.getStatus() == Status.PUBLISHED || performance.getStatus() == Status.CANCELED) {
      throw new InvalidParameterException("Cannot delete performance that has been published or cancled");
    }
  ...
  private Performance buildNewPerformance(PerformanceInfo info)
      throws InvalidParameterException, EntityNotFoundException {
    Performance performance = Performance.newBuilder().setTitle(info.title()).setDescription(info.description())
        .setCity(info.city()).setCountry(info.country()).setState(info.state()).setVenue(info.venue())
        .setDateTime(info.dateTime()).build();
  ...
  private Performance addNewSections(Performance performance, List<PerformanceSectionInfo> program)
    ...
    for (PerformanceSectionInfo info : program) {
      PerformanceSection performanceSection = PerformanceSection.newBuilder().setPrimaryCast(info.primaryCast())
          .setSectionPosition(info.sectionPosition()).build();
  ...
  private Performance deleteSections(Performance performance, List<PerformanceSectionInfo> deleteSections)
      throws InvalidParameterException, EntityNotFoundException {
  ...
  private Performance editExistingPerformanceSections(Performance performance,
      List<PerformanceSectionInfo> editSections) throws InvalidParameterException, EntityNotFoundException {
    for (PerformanceSectionInfo info : editSections) {
      PerformanceSection performanceSection = performance.getPerformanceSectionById(info.id()).toBuilder()
          .setPrimaryCast(info.primaryCast()).setSectionPosition(info.sectionPosition()).build();
      ...
      performanceSection.getPerformanceCastMembers()
  ...
  private PerformanceSection addNewCastMembers(PerformanceSection performanceSection,
      List<PerformancePositionInfo> performancePositions) throws InvalidParameterException, EntityNotFoundException {
    for (PerformancePositionInfo positionInfo : performancePositions) {
      ...
      for (PerformanceCastInfo castsInfo : positionInfo.performanceCasts()) {
        ...
        for (PerformanceCastMemberInfo memberInfo : castsInfo.performanceCastMembers()) {
          performanceSection = addPerformanceCastMember(performanceSection, memberInfo, currentPosition,
              currentCastNumber, isPerforming);
        }
      }
    }
  ...
  private PerformanceSection addPerformanceCastMember(PerformanceSection performanceSection,
      PerformanceCastMemberInfo memberInfo, Position position, int castNumber, boolean isPerforming)
      throws InvalidParameterException, EntityNotFoundException {

    User user = userService.getUser(memberInfo.userId());

    PerformanceCastMember member = PerformanceCastMember.newBuilder().setOrder(memberInfo.order())
        .setCastNumber(castNumber).build();
  ...
  private PerformanceSection updatePerformanceCastMember(PerformanceSection performanceSection,
    ...
    performanceSection.getPerformanceCastMemberById(memberInfo.id()).toBuilder().setOrder(memberInfo.order())
        .setCastNumber(castNumber).build();
      ...
      for (PerformanceCastInfo castsInfo : positionInfo.performanceCasts()) {
        ...
        for (PerformanceCastMemberInfo memberInfo : castsInfo.performanceCastMembers()) {
          if (memberInfo.delete() != null && memberInfo.delete()) {
            ...
            updatePerformanceCastMember(performanceSection, memberInfo, currentPosition, currentCastNumber);
          }
        }
      }
    }
  ...
  private PerformanceSection deletePerformanceCastMember(PerformanceSection performanceSection,
      PerformanceCastMemberInfo info) throws InvalidParameterException, EntityNotFoundException {
  ...
  private Performance updatePerformance(PerformanceInfo info)
      throws InvalidParameterException, EntityNotFoundException {
    Performance performance = getPerformance(info.id()).toBuilder().setTitle(info.title())
        .setDescription(info.description()).setCity(info.city()).setCountry(info.country()).setState(info.state())
        .setVenue(info.venue()).setDateTime(info.dateTime()).build();
  ...
  private List<String> verifyPerformance(Performance performance) throws InvalidParameterException {
    ...
    for (PerformanceSection section : performance.getProgram()) {
      ...
      if (uniqueSectionPositions.contains(order)) {
        throw new InvalidParameterException("All Performance Sections must have unique section positions");
      }
      ...
      for (PerformanceCastMember member : section.getPerformanceCastMembers()) {
        ...
        if (cast.contains(user.getId())) {
          warnings.add(String.format("User %s %s appears multiple times in cast %d", user.getFirstName(),
              user.getLastName(), castNumber));
        } else {
          cast.add(user.getId());
        }
      }
  ...
  public PerformanceServices(PerformanceRepository performanceRepo, SectionServices sectionService,
